...
class BaboMovieRoomDatabaseTest {
    ...
    fun simpleInsert() {
        ...
        Assert.assertEquals(1, baboMovieDao?.getEntries()?.size)
        Assert.assertEquals(1, baboMovieDao?.getLiked()?.size)
        Assert.assertEquals(0, baboMovieDao?.getDisliked()?.size)
        Assert.assertEquals(0, baboMovieDao?.getSuperliked()?.size)

        baboMovieDao?.delete(newMovie)

        Assert.assertEquals(0, baboMovieDao?.getEntries()?.size)
        Assert.assertEquals(0, baboMovieDao?.getLiked()?.size)
        Assert.assertEquals(0, baboMovieDao?.getDisliked()?.size)
        Assert.assertEquals(0, baboMovieDao?.getSuperliked()?.size)
    }
}